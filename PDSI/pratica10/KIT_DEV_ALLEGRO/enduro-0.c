/*========================================== Prática com ALLEGRO ======================================
10. enduro-0.c​: desenhe um carro na pista e o faça movimentar da direita para a
esquerda sem que os limites da pista sejam ultrapassados.
=======================================================================================================*/
...
void init_global_vars() {
	...
	CAR_DX = (TRACK_BOTTOM_RIGHT_X - TRACK_BOTTOM_LEFT_X)*0.005;
...
void draw_car(Car carro) {
	ALLEGRO_COLOR cor = al_map_rgb(0, 0, 0);
	al_draw_filled_rectangle(carro.x-CAR_W/2, carro.y - CAR_H/2, carro.x+CAR_W/2, carro.y+CAR_H/2, carro.cor);
	al_draw_filled_rectangle(carro.x-CAR_W/2-WHEEL_W, carro.y-WHEEL_H/2,
							 carro.x-CAR_W/2, carro.y+WHEEL_H/2, cor);
	al_draw_filled_rectangle(carro.x+CAR_W/2, carro.y-WHEEL_H/2,
							 carro.x+CAR_W/2+WHEEL_W, carro.y+WHEEL_H/2, cor);
}

void init_car(Car *carro) {

    carro->x = SCREEN_W/2;
	carro->y = TRACK_BOTTOM_RIGHT_Y - WHEEL_H/2;
	carro->cor = al_map_rgb(255,0,0);
	carro->accel = 0;
	carro->breaking = 0;
	carro->left = 0;
	carro->right = 0;
...
void update_car(Car *carro) {
	float carSize = CAR_W+2*WHEEL_W;
	if(carro->x + carSize/2 < TRACK_BOTTOM_RIGHT_X) 
		carro->x += carro->right*CAR_DX;
	if(carro->x - carSize/2 > TRACK_BOTTOM_LEFT_X) 
		carro->x -= (carro->left)*CAR_DX;	
}
